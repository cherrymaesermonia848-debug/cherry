...
export default function GalleryPage() {
  ...
  const uploadBulk = (event: FormEvent<HTMLFormElement>) => {
    ...
    setMessage(`${images.length} image${images.length === 1 ? "" : "s"} ready for bulk upload.`);
  };
          ...
          <div className="flex flex-col gap-3 border-b border-zinc-200 px-4 py-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            {images.length > 0 ? (
              <div className="flex flex-col gap-2 sm:flex-row">
                <button
                  type="submit"
                  className="h-10 rounded-md bg-teal-700 px-4 text-sm font-semibold text-white transition hover:bg-teal-800 focus:outline-none focus:ring-2 focus:ring-teal-600 focus:ring-offset-2"
                >
                  Upload Bulk
                </button>
                <button
